...
export const CombinationBuilder: React.FC<CombinationBuilderProps> = ({ models, combinations, onSave, onDelete }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
      ...
      <div className="lg:col-span-1 bg-slate-900 rounded-xl p-6 border border-slate-800 overflow-y-auto">
        ...
        <div className="space-y-4">
          {combinations.map(combo => {
            ...
            return (
              <div 
                ...
                className={`p-4 rounded-lg border transition-all cursor-pointer group ${isEditing ? 'bg-indigo-900/20 border-indigo-500 shadow-md shadow-indigo-900/20' : 'bg-slate-800 border-slate-700 hover:border-indigo-500 hover:bg-slate-800/80'}`}
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className={`font-semibold ${isEditing ? 'text-indigo-300' : 'text-slate-100'}`}>{combo.name}</h3>
                  <button 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                      onDelete(combo.id); 
                    ...
                    className="text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-slate-700 rounded"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
                <p className="text-sm text-slate-400 mb-3 line-clamp-2">{combo.description}</p>
                <div className="space-y-2 text-xs text-slate-500">
                  <div className="flex items-center gap-2">
                    <span className="bg-indigo-900/50 text-indigo-300 px-1.5 py-0.5 rounded">CP</span>
                    {cp?.name || 'Unknown'}
                  </div>
                  {combo.loraIds.length > 0 && (
                    <div className="flex items-center gap-2">
                      <span className="bg-purple-900/50 text-purple-300 px-1.5 py-0.5 rounded">LoRA</span>
                      {combo.loraIds.length} active
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2">
                    {combo.vaeId && (
                        <span className="bg-emerald-900/30 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-900/50">VAE</span>
                    )}
                    {combo.clipId && (
                        <span className="bg-pink-900/30 text-pink-400 px-1.5 py-0.5 rounded border border-pink-900/50">CLIP</span>
                    )}
                    {combo.textEncoderId && (
                        <span className="bg-orange-900/30 text-orange-400 px-1.5 py-0.5 rounded border border-orange-900/50">TE</span>
                    )}
                  </div>
                  ...
                  {combo.triggerWords && combo.triggerWords.length > 0 && (
                     <div className="flex flex-wrap gap-1 mt-1">
                        {combo.triggerWords.map((tw, i) => (
                          <span key={i} className="px-1.5 py-0.5 bg-slate-700 rounded text-[10px] text-slate-300">{tw}</span>
                        ))}
                     </div>
                  )}
                </div>
          ...
          {combinations.length === 0 && (
            <div className="text-slate-600 text-center py-8">No recipes saved yet.</div>
          )}
        </div>
      </div>
      ...
      <div className="lg:col-span-2 bg-slate-900 rounded-xl p-6 border border-slate-800 overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
            ...
            {editingId && (
                <button 
                  onClick={handleClear} 
                  className="text-sm bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg flex items-center gap-2 transition-colors border border-slate-700"
                >
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
             <div>
               <label className="block text-sm font-medium text-slate-400 mb-1">Recipe Name</label>
               <input 
                 type="text" 
                 value={comboName}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-1">Trigger Words (Combined)</label>
            <input 
              type="text" 
              value={triggerWords}
              onChange={e => setTriggerWords(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-slate-100 focus:outline-none focus:border-indigo-500 placeholder-slate-600"
              placeholder="e.g. detailed, 8k, masterpiece (comma separated)"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-indigo-400 mb-2">1. Select Checkpoint (Base)</label>
            <select 
              value={selectedCheckpoint}
              onChange={e => setSelectedCheckpoint(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-slate-100 focus:outline-none focus:border-indigo-500"
            >
          ...
          <div>
            <label className="block text-sm font-medium text-pink-400 mb-2">2. Add LoRAs (Optional)</label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 bg-slate-950/50 rounded-lg border border-slate-800">
              {loras.map(lora => (
                <div 
                  key={lora.id}
                  onClick={() => handleToggleLora(lora.id)}
                  className={`flex items-center justify-between p-2 rounded cursor-pointer transition-colors ${selectedLoras.includes(lora.id) ? 'bg-indigo-900/30 border border-indigo-500/50' : 'hover:bg-slate-800 border border-transparent'}`}
                >
                  <span className="text-sm truncate">{lora.name}</span>
                  {selectedLoras.includes(lora.id) && (
                     <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                       <input 
                         ...
                         className="w-16 bg-slate-950 border border-slate-700 rounded px-1 py-0.5 text-xs text-right"
                       />
                     </div>
                  )}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-emerald-400 mb-2">3. Additional Models (Optional)</label>
            ...
            <div className="bg-slate-950/50 p-4 rounded-lg border border-slate-800 h-fit">
          ...
          <button
            ...
            className={`w-full text-white font-semibold py-3 rounded-lg transition-all shadow-lg flex items-center justify-center gap-2 mt-4 
              ${editingId 
                ...
                : 'bg-indigo-600 hover:bg-indigo-500 shadow-indigo-900/50 disabled:opacity-50 disabled:cursor-not-allowed'}`}
          >
