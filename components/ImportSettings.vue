<template>
    <div class="max-w-2xl mx-auto space-y-8">
        <!-- Tab Switcher -->
        <div class="flex p-1 mx-auto mb-6 rounded-lg bg-neutral-100 dark:bg-neutral-800 w-fit">
            <button
                @click="activeTab = 'db'"
                :class="[
                    'px-6 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2',
                    activeTab === 'db' ? 'bg-white dark:bg-neutral-700 text-blue-600 dark:text-white shadow-sm' : 'text-neutral-500 hover:text-neutral-900 dark:hover:text-white',
                ]">
                <Database :size="16" />
                Import Database
            </button>
            <button
                ...
                :class="[
                    'px-6 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2',
                    activeTab === 'file' ? 'bg-white dark:bg-neutral-700 text-blue-600 dark:text-white shadow-sm' : 'text-neutral-500 hover:text-neutral-900 dark:hover:text-white',
                ]">
                <FileType :size="16" />
                Import Model File
            </button>
        ...
        <div class="p-8 transition-colors bg-white border shadow-sm border-neutral-200 dark:bg-neutral-900 rounded-xl dark:border-neutral-800 dark:shadow-none">
            <!-- DB Import Tab -->
            <template v-if="activeTab === 'db'">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-neutral-500 dark:text-neutral-400">
                            Upload your
                            <code>invokeai.db</code>
                            file to import models.
                        </p>
                    </div>
                </div>

                <!-- Strategy Selection -->
                <div class="p-4 mb-6 border rounded-lg border-neutral-200 bg-neutral-50 dark:bg-neutral-950/50 dark:border-neutral-800">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <button
                            ...
                            <div class="flex items-center justify-center w-8 h-8 rounded-full bg-neutral-200 dark:bg-neutral-800">
                                <FileUp :size="16" />
                            </div>
                            None
                        </button>
                ...
                <div
                    ...
                    <input type="file" id="db-upload" accept=".db,.sqlite" @change="handleDbUploadChange" class="hidden" />
                    <label
                        for="db-upload"
                        :class="['cursor-pointer flex flex-col items-center gap-3', status === 'loading' || status === 'processing' ? 'pointer-events-none opacity-50' : '']">
                        <div
                            class="flex items-center justify-center w-16 h-16 transition-colors rounded-full text-neutral-400 bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-400 group-hover:text-blue-600 dark:group-hover:text-white">
                ...
                <div v-if="status === 'processing'" class="mt-6">
                    ...
                    <div class="w-full h-2 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800">
                        <div class="h-full transition-all duration-300 bg-blue-500" :style="{ width: `${(processedCount / totalToProcess) * 100}%` }" />
                    </div>
                ...
                <div
                    v-if="status === 'success'"
                    class="flex items-start gap-3 p-4 mt-6 border rounded-lg bg-emerald-100 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-900/50">
            ...
            <template v-else>
                ...
                <div
                    ...
                    <input type="file" id="file-upload" accept=".safetensors,.ckpt,.pt,.bin" @change="handleModelFileChange" class="hidden" />
                    <label
                        for="file-upload"
                        :class="[
                        ...
                        <div
                            class="flex items-center justify-center w-16 h-16 transition-colors rounded-full text-neutral-400 bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-400 group-hover:text-pink-600 dark:group-hover:text-white">
                ...
                <div v-if="fileStatus !== 'idle'" class="mt-6 space-y-4 animate-in fade-in slide-in-from-bottom-2">
                    <!-- Hash Display -->
                    <div class="flex items-center justify-between p-4 rounded-lg bg-neutral-100 dark:bg-neutral-800">
                        <div class="flex items-center gap-3 overflow-hidden">
                            <div class="flex items-center justify-center w-8 h-8 rounded-full bg-neutral-200 dark:bg-neutral-700 shrink-0">
                                <Hash :size="14" class="text-neutral-600 dark:text-neutral-400" />
                            </div>
                            <div class="flex flex-col min-w-0">
                                <span class="text-xs font-bold uppercase text-neutral-500">SHA256 Hash</span>
                                <code class="font-mono text-xs truncate text-neutral-700 dark:text-neutral-300">
                        ...
                        <div v-if="fileHash" class="flex gap-2 shrink-0">
                            <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                class="p-2 rounded text-neutral-500 hover:bg-white dark:hover:bg-neutral-600 hover:text-blue-500"
                            ...
                            <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                class="p-2 rounded text-neutral-500 hover:bg-white dark:hover:bg-neutral-600 hover:text-green-500"
                    ...
                    <div
                        v-if="fileStatus === 'ready' && scannedModel"
                        class="flex flex-col overflow-hidden bg-white border shadow-lg border-neutral-200 dark:bg-neutral-800 dark:border-neutral-700 rounded-xl md:flex-row">
        ...
        <div class="p-6 transition-colors bg-white border shadow-sm border-neutral-200 dark:bg-neutral-900 rounded-xl dark:border-neutral-800 dark:shadow-none">
...
import { Database, CheckCircle2, AlertCircle, FileUp, Image as ImageIcon, Search, Sparkles, FileType, Hash, ExternalLink, Plus, Loader2 } from 'lucide-vue-next';
