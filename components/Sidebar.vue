    ...
    >
      ...
      <nav class="flex-1 px-3 py-4 space-y-2 overflow-x-hidden overflow-y-auto">
        ...
        >
          ...
          <span class="hidden ml-0 overflow-hidden font-medium transition-all duration-300 group-hover:inline-block group-hover:ml-3">
            {{ item.label }}
          </span>
        </Button>
      </nav>
      ...
      <div class="flex flex-col items-center flex-shrink-0 gap-2 p-4 overflow-hidden border-t text-neutral-500 border-neutral-200 dark:border-neutral-800 group-hover:items-start whitespace-nowrap">
        <Button
        ...
        >
          ...
          <Moon v-else :size="20" />
          <span class="hidden ml-0 overflow-hidden text-sm font-medium transition-all duration-300 group-hover:inline-block group-hover:ml-3">
            {{ isDarkMode ? 'Light Mode' : 'Dark Mode' }}
          </span>
        </Button>

        <div v-if="user" class="flex items-center justify-center w-full p-2 mt-1 transition-colors rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 group-hover:justify-start">
          <img :src="user.picture" :alt="user.name" class="flex-shrink-0 w-5 h-5 rounded-full ring-2 ring-blue-500" />
          <div class="flex flex-col items-start w-0 ml-0 overflow-hidden transition-all duration-300 opacity-0 group-hover:ml-3 group-hover:opacity-100 group-hover:w-auto">
        ...
        >
          <LogIn :size="20" />
          <span class="hidden ml-0 overflow-hidden text-sm font-medium transition-all duration-300 group-hover:inline-block group-hover:ml-3"> Sign In </span>
        </Button>
